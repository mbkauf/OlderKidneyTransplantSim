...
fwrite(m_ddtx_coef, paste0(out_path, "Stage2/part3/ddtx_coef.csv"))
fwrite(m_ldtx_coef, paste0(out_path, "Stage2/part3/ldtx_coef.csv"))
fwrite(m_mort_coef, paste0(out_path, "Stage2/part3/mort_coef.csv"))
fwrite(m_remove_coef, paste0(out_path, "Stage2/part3/remove_coef.csv"))
...
fwrite(mlogit_coef, paste0(out_path, "Stage2/part3/mlogit_coef.csv"))
fwrite(gs_mort_coef_rand, paste0(out_path, "Stage2/part3/gs_mort_coef.csv"))
fwrite(gl_coef_rand, paste0(out_path, "Stage2/part3/gl_coef.csv"))
fwrite(dial_mort_coef_rand, paste0(out_path, "Stage2/part3/dial_mort_coef.csv"))
fwrite(gl_mort_coef, paste0(out_path, "Stage2/part3/gl_mort_coef.csv"))


### Start here: Load parameter sets ###
# Waitlist equations
m_ddtx_coef_samp   <- fread(paste0(out_path, "Stage2/part3/ddtx_coef.csv"))[,-1]
m_ldtx_coef_samp   <- fread(paste0(out_path, "Stage2/part3/ldtx_coef.csv"))[,-1]
m_mort_coef_samp   <- fread(paste0(out_path, "Stage2/part3/mort_coef.csv"))[,-1]
m_remove_coef_samp <- fread(paste0(out_path, "Stage2/part3/remove_coef.csv"))[,-1]

m_ddtx_shape_samp   <- as.matrix(m_ddtx_coef_samp[, 27], ncol = 1)
m_ldtx_shape_samp   <- as.matrix(m_ldtx_coef_samp[, 27], ncol = 1)
m_mort_shape_samp   <- as.matrix(m_mort_coef_samp[, 27], ncol = 1)
m_remove_shape_samp <- as.matrix(m_remove_coef_samp[, 27], ncol = 1)

m_ddtx_coef_samp   <- as.matrix(m_ddtx_coef_samp[, 1:26])
m_ldtx_coef_samp   <- as.matrix(m_ldtx_coef_samp[, 1:26])
m_mort_coef_samp   <- as.matrix(m_mort_coef_samp[, 1:26])
m_remove_coef_samp <- as.matrix(m_remove_coef_samp[, 1:26])

# Post-transplant equations
mlogit_coef    <- fread(paste0(out_path, "Stage2/part3/mlogit_coef.csv"))
gl_coef        <- fread(paste0(out_path, "Stage2/part3/gl_coef.csv"))
gs_mort_coef   <- fread(paste0(out_path, "Stage2/part3/gs_mort_coef.csv"))
dial_mort_coef <- fread(paste0(out_path, "Stage2/part3/dial_mort_coef.csv"))
gl_mort_coef   <- fread(paste0(out_path, "Stage2/part3/gl_mort_coef.csv"))

gl_shape        <- as.matrix(gl_coef[, 20], ncol = 1)
gs_mort_shape   <- as.matrix(gs_mort_coef[, 19], ncol = 1)
dial_mort_shape <- as.matrix(dial_mort_coef[, 21], ncol = 1)

mlogit_coef    <- as.matrix(mlogit_coef)
gl_coef        <- as.matrix(gl_coef[, 1:19])
gs_mort_coef   <- as.matrix(gs_mort_coef[, 1:18])
dial_mort_coef <- as.matrix(dial_mort_coef[, 1:20])
gl_mort_coef   <- as.matrix(gl_mort_coef)
...
process_run <- function(list_df = list_sim_df,
                        full_df = sim_df) {
  ### Assess waitlist fit
  ## Clean and run survival models
  list_df <- lazy_dt(list_df) %>%
  ...
  df_diab <- split(list_df, list_df$diab_stat)
  df_race <- split(list_df, list_df$race)

  ### Deceased donor transplant
  ddtx_km_fit          <- survfit(Surv(time = months_to_event, ddtx==1) ~ 1, data = list_df)
  ...
  # Parametric Fits
  ddtx_shape_all      <- flexsurvreg(Surv(time = months_to_event, ddtx==1)~1, data=list_df, dist="weibull")$coefficients
  ...
  ldtx_km_fit          <- survfit(Surv(time = months_to_event, ldtx==1) ~ 1, data = list_df)
  ...
  # Parametric Fits
  ldtx_shape_all      <- flexsurvreg(Surv(time = months_to_event, ldtx==1)~1, data=list_df, dist="gompertz")$coefficients
  ...
  ### Waitlist mortality
  mort_km_fit          <- survfit(Surv(time = months_to_event, mort==1) ~ 1, data = list_df)
  ...
  # Parametric Fits
  mort_shape_all      <- flexsurvreg(Surv(time = months_to_event, mort==1)~1, data=list_df, dist="weibull")$coefficients
  ...
  ### Waitlist removal
  remove_km_fit          <- survfit(Surv(time = months_to_event, remove==1) ~ 1, data = list_df)
  ...
  # Parametric Fits
  remove_shape_all      <- flexsurvreg(Surv(time = months_to_event, remove==1)~1, data=list_df, dist="weibull")$coefficients
  ...
  ### Assess post-tx fit
  full_df <- lazy_dt(full_df) %>% filter(ddtx==1) %>%
    as_tibble()
  ...
  for(i in 0:3) {
    prop_30day_all[i+1] <- nrow(full_df[full_df$tx_outcome==i,])
    prop_30day_diab0[i+1] <- nrow(full_df[full_df$tx_outcome==i & full_df$diab_stat==0,])
    prop_30day_diab1[i+1] <- nrow(full_df[full_df$tx_outcome==i & full_df$diab_stat==1,])
    prop_30day_white[i+1] <- nrow(full_df[full_df$tx_outcome==i & full_df$race==0,])
    prop_30day_black[i+1] <- nrow(full_df[full_df$tx_outcome==i & full_df$race==1,])
    prop_30day_hispanic[i+1] <- nrow(full_df[full_df$tx_outcome==i & full_df$race==2,])
    prop_30day_other[i+1] <- nrow(full_df[full_df$tx_outcome==i & full_df$race==3,])
  }
  ...
  # } else {
  #   prop_30day[1,]   <- as.vector(table(full_df$tx_outcome))
  #   prop_30day[2:3,] <- table(full_df$diab_stat, full_df$tx_outcome)
  #   prop_30day[4:7,] <- table(full_df$race, full_df$tx_outcome)
  # }
  ...
  ## Split data
  df_diab <- split(full_df, full_df$diab_stat)
  df_race <- split(full_df, full_df$race)

  ### Death with function
  # KM Curves
  gs_mort_fit          <- survival::survfit(Surv(time = gs_death_time, gs_death==1) ~ 1, data = full_df)
  ...
  gs_mort_shape_all      <- flexsurv::flexsurvreg(Surv(time = gs_death_time, gs_death==1) ~ 1,
                                                  inits = c(shape = 0.001, rate = 0.001), data=full_df, dist="gompertz")$coefficients
  ...
  v_gs_mort_shape <- c(gs_mort_shape_all,
                       ...
                       gs_mort_shape_other)
  ### Graft loss
  gl_fit          <- survival::survfit(Surv(time = gl_time, graft_loss==1) ~ 1, data = full_df)
  ...
  # Initialize vectors
  gl_mort_surv       <- rep(NA, 20)
  gl_mort_surv_diab0 <- rep(NA, 20)
  gl_mort_surv_diab1 <- rep(NA, 20)
  gl_mort_surv_white <- rep(NA, 20)
  gl_mort_surv_black <- rep(NA, 20)
  gl_mort_surv_hispanic <- rep(NA, 20)
  gl_mort_surv_other <- rep(NA, 20)

  tryCatch(expr = {
    gl_mort_fit  <- survival::survfit(Surv(time = gl_death_time, gl_death==1) ~ 1, data = full_df)
  ...
  # Parametric Fits
  tryCatch(expr = {
    sim_df_weibull <- lazy_dt(full_df) %>%
  ...
  # Combine to export
  return(list(ddtx_surv, ldtx_surv,
...
### Run full 500k and store results on server ###
...
pb <- txtProgressBar(min=7000, max=499900, style=3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress=progress)

# 499000
start_time <- Sys.time()
out <- foreach(i = seq(from = 0, to = 499900, by = 100), .combine='comb',
               .multicombine=TRUE, .options.snow=opts,
               .packages = c("MASS", "copula", "psych", "dtplyr", "survival",
                             "flexsurv", "dplyr", "data.table")) %dopar% {
                               ...
                               for(j in 1:100) {
                                 ...
                                 list_sim_df <- list.simulation(
                                   ...
                                   data = sample_df,
                                   ddtx_coef = m_ddtx_coef_samp[k, ],
                                   ddtx_p = m_ddtx_shape_samp[k, 1],
                                   ldtx_coef = m_ldtx_coef_samp[k, ],
                                   ldtx_gamma = m_ldtx_shape_samp[k, 1],
                                   mort_coef = m_mort_coef_samp[k, ],
                                   mort_shape = m_mort_shape_samp[k, 1],
                                   remove_coef = m_remove_coef_samp[k, ],
                                   remove_p = m_remove_shape_samp[k, 1]
                                 )

                                 sim_df <- run.simulation(
                                   data = list_sim_df,
                                   seed = round(runif(1, min = 1, max = 99999)),
                                   gl30_coef = mlogit_coef[k, 1:18],
                                   dgf_coef = mlogit_coef[k, 19:36],
                                   mort30_coef = mlogit_coef[k, 37:54],
                                   gl_coef = gl_coef[k, ],
                                   gl_shape = gl_shape[k, 1],
                                   gs_mort_coef = gs_mort_coef[k, ],
                                   gs_mort_shape = gs_mort_shape[k, 1],
                                   dial_mort_coef = dial_mort_coef[k, ],
                                   dial_mort_shape = dial_mort_shape[k, 1],
                                   gl_mort_coef = gl_mort_coef[k, ]
                                 )

                                 results_list[[j]] <- process_run()
                               }

                               big_results <- do.call(Map, c(f = rbind, results_list))

                               for(x in 1:length(big_results)){
                                 fwrite(big_results[[x]],
                                        paste0("C:/Users/mbkauf/Documents/Calibration/Recalibration/",
                                               i, "/out_", x, ".csv"))
                               }
                               out <- NULL
                               print(k)
...
m_gof <- matrix(nrow = 500000, ncol = 14)
...
pb <- txtProgressBar(min = 0, max = 500000, style = 3)
for (i in 1:500000) {
  m_gof[i,1] <- i
  l_gof <- get_gof_all_likelihood(targets = l_target, sim_results = l_out, iter = i)
  m_gof[i,2] <- l_gof[[1]]
  m_gof[i,3] <- l_gof[[2]]
  m_gof[i,4] <- l_gof[[3]]
  m_gof[i,5] <- l_gof[[4]]
...
fwrite(m_gof, paste0(out_path, "Stage2/part3/m_gof.csv"))

##### Read and combine #####
m_gof1 <- fread(file = paste0(out_path, "Stage2/part1/m_gof.csv"))
m_gof2 <- fread(file = paste0(out_path, "Stage2/part2/m_gof.csv"))
m_gof3 <- as.data.frame(m_gof)
# m_gof4 <-
# m_gof2$id <- m_gof2$id + n

m_gof_all <- rbind(m_gof1, m_gof2, m_gof3)
m_gof_all$id <- seq(from = 1, to = 600000)

df_gof <- lazy_dt(m_gof_all) %>%
  mutate(nll_surv_norm_post = gof_post_surv) %>%
  mutate(nll_shape_norm_post = gof_post_shape) %>%
  mutate(gof_surv_post = nll_surv_norm_post + gof_30 + gof_gl_mort_prop) %>%
  mutate(gof_shape_post = nll_shape_norm_post + gof_30 + gof_gl_mort_prop) %>%
  mutate(rank_surv_post = rank(gof_surv_post)) %>%
  mutate(rank_shape_post = rank(gof_shape_post))%>%
  mutate(w_surv_post = 1/(gof_surv_post)) %>%
  mutate(w_shape_post = 1/(gof_shape_post)) %>%
  mutate(w_surv_post = w_surv_post/sum(w_surv_post, na.rm = TRUE)) %>%
  mutate(w_shape_post = w_shape_post/sum(w_shape_post, na.rm = TRUE)) %>%
  # mutate(nll_surv_norm_wait = gof_wait_surv/1680) %>%
  # mutate(nll_shape_norm_wait = gof_wait_shape/28) %>%
  # mutate(gof_surv_wait = nll_surv_norm_wait + gof_table1) %>%
  # mutate(gof_shape_wait = nll_shape_norm_wait + gof_table1) %>%
  # mutate(gof_surv_all = gof_surv_wait + gof_surv_post) %>%
  # mutate(gof_shape_all = gof_shape_wait + gof_shape_post) %>%
  # mutate(rank_surv_all = rank(gof_surv_all)) %>%
  # mutate(rank_shape_all = rank(gof_shape_all)) %>%
  # mutate(w_surv_all = 1/exp(gof_surv_all)) %>%
  # mutate(w_shape_all = 1/exp(gof_shape_all)) %>%
  # mutate(w_surv_all = w_surv_all/sum(w_surv_all, na.rm = TRUE)) %>%
# mutate(w_shape_all = w_shape_all/sum(w_shape_all, na.rm = TRUE))  %>%
as_tibble()


fwrite(df_gof, paste0(out_path, "Stage2/part3/df_gof.csv"))

# m_ddtx_coef1   <- fread(paste0(out_path, "Stage2/part1/ddtx_coef.csv"))
# m_ldtx_coef1   <- fread(paste0(out_path, "Stage2/part1/ldtx_coef.csv"))
# m_mort_coef1   <- fread(paste0(out_path, "Stage2/part1/mort_coef.csv"))
# m_remove_coef1 <- fread(paste0(out_path, "Stage2/part1/remove_coef.csv"))
# m_ddtx_coef2   <- fread(paste0(out_path, "Stage2/part2/ddtx_coef.csv"))
# m_ldtx_coef2   <- fread(paste0(out_path, "Stage2/part2/ldtx_coef.csv"))
# m_mort_coef2   <- fread(paste0(out_path, "Stage2/part2/mort_coef.csv"))
# m_remove_coef2 <- fread(paste0(out_path, "Stage2/part2/remove_coef.csv"))
#
# mlogit_coef1    <- fread(paste0(out_path, "Stage2/part1/mlogit_coef.csv"))
# gs_mort_coef1   <- fread(paste0(out_path, "Stage2/part1/gs_mort_coef.csv"))
# gl_coef1        <- fread(paste0(out_path, "Stage2/part1/gl_coef.csv"))
# dial_mort_coef1 <- fread(paste0(out_path, "Stage2/part1/dial_mort_coef.csv"))
# gl_mort_coef1   <- fread(paste0(out_path, "Stage2/part1/gl_mort_coef.csv"))
# mlogit_coef2    <- fread(paste0(out_path, "Stage2/part2/mlogit_coef.csv"))
# gs_mort_coef2   <- fread(paste0(out_path, "Stage2/part2/gs_mort_coef.csv"))
# gl_coef2        <- fread(paste0(out_path, "Stage2/part2/gl_coef.csv"))
# dial_mort_coef2 <- fread(paste0(out_path, "Stage2/part2/dial_mort_coef.csv"))
# gl_mort_coef2   <- fread(paste0(out_path, "Stage2/part2/gl_mort_coef.csv"))
#
# m_ddtx_coef_all    <- rbind(m_ddtx_coef1, m_ddtx_coef2)
# m_ldtx_coef_all    <- rbind(m_ldtx_coef1, m_ldtx_coef2)
# m_mort_coef_all    <- rbind(m_mort_coef1, m_mort_coef2)
# m_remove_coef_all  <- rbind(m_remove_coef1, m_remove_coef2)
# mlogit_coef_all    <- rbind(mlogit_coef1, mlogit_coef2)
# gs_mort_coef_all   <- rbind(gs_mort_coef1, gs_mort_coef2)
# gl_coef_all        <- rbind(gl_coef1, gl_coef2)
# dial_mort_coef_all <- rbind(dial_mort_coef1, dial_mort_coef2)
# gl_mort_coef_all   <- rbind(gl_mort_coef1, gl_mort_coef2)
#
# m_ddtx_coef_all$id <- seq.int(100000)
# m_ldtx_coef_all$id <- seq.int(100000)
# m_mort_coef_all$id <- seq.int(100000)
# m_remove_coef_all$id <- seq.int(100000)
# mlogit_coef_all$id <- seq.int(100000)
# gs_mort_coef_all$id <- seq.int(100000)
# gl_coef_all$id <- seq.int(100000)
# dial_mort_coef_all$id <- seq.int(100000)
# gl_mort_coef_all$id <- seq.int(100000)
#
# fwrite(m_ddtx_coef_all, paste0(out_path, "Stage2/ddtx_coef.csv"))
# fwrite(m_ldtx_coef_all, paste0(out_path, "Stage2/ldtx_coef.csv"))
# fwrite(m_mort_coef_all, paste0(out_path, "Stage2/mort_coef.csv"))
# fwrite(m_remove_coef_all, paste0(out_path, "Stage2/remove_coef.csv"))
# fwrite(mlogit_coef_all, paste0(out_path, "Stage2/mlogit_coef.csv"))
# fwrite(gs_mort_coef_all, paste0(out_path, "Stage2/gs_mort_coef.csv"))
# fwrite(gl_coef_all, paste0(out_path, "Stage2/gl_coef.csv"))
# fwrite(dial_mort_coef_all, paste0(out_path, "Stage2/dial_mort_coef.csv"))
# fwrite(gl_mort_coef_all, paste0(out_path, "Stage2/gl_mort_coef.csv"))

ddtx_coef_1      <- fread(file = paste0(out_path, "Stage2/ddtx_coef.csv"))
ldtx_coef_1      <- fread(file = paste0(out_path, "Stage2/ldtx_coef.csv"))
mort_coef_1      <- fread(file = paste0(out_path, "Stage2/mort_coef.csv"))
remove_coef_1    <- fread(file = paste0(out_path, "Stage2/remove_coef.csv"))
mlogit_coef_1    <- fread(file = paste0(out_path, "Stage2/mlogit_coef.csv"))
gs_mort_coef_1   <- fread(file = paste0(out_path, "Stage2/gs_mort_coef.csv"))
gl_coef_1        <- fread(file = paste0(out_path, "Stage2/gl_coef.csv"))
dial_mort_coef_1 <- fread(file = paste0(out_path, "Stage2/dial_mort_coef.csv"))
gl_mort_coef_1   <- fread(file = paste0(out_path, "Stage2/gl_mort_coef.csv"))

ddtx_coef_2      <- fread(file = paste0(out_path, "Stage2/part3/ddtx_coef.csv"))
ldtx_coef_2      <- fread(file = paste0(out_path, "Stage2/part3/ldtx_coef.csv"))
mort_coef_2      <- fread(file = paste0(out_path, "Stage2/part3/mort_coef.csv"))
remove_coef_2    <- fread(file = paste0(out_path, "Stage2/part3/remove_coef.csv"))
mlogit_coef_2    <- fread(file = paste0(out_path, "Stage2/part3/mlogit_coef.csv"))
gl_coef_2        <- fread(file = paste0(out_path, "Stage2/part3/gl_coef.csv"))
gs_mort_coef_2   <- fread(file = paste0(out_path, "Stage2/part3/gs_mort_coef.csv"))
dial_mort_coef_2 <- fread(file = paste0(out_path, "Stage2/part3/dial_mort_coef.csv"))
gl_mort_coef_2   <- fread(file = paste0(out_path, "Stage2/part3/gl_mort_coef.csv"))

ddtx_coef_2$id      <- ddtx_coef_2$id + 100000
ldtx_coef_2$id      <- ldtx_coef_2$id + 100000
mort_coef_2$id      <- mort_coef_2$id + 100000
remove_coef_2$id    <- remove_coef_2$id + 100000

mlogit_coef_2$id    <- seq.int(from = 100001, to = 600000)
gl_coef_2$id        <- seq.int(from = 100001, to = 600000)
gs_mort_coef_2$id   <- seq.int(from = 100001, to = 600000)
dial_mort_coef_2$id <- seq.int(from = 100001, to = 600000)
gl_mort_coef_2$id   <- seq.int(from = 100001, to = 600000)

m_ddtx_coef_all    <- rbind(ddtx_coef_1, ddtx_coef_2)
m_ldtx_coef_all    <- rbind(ldtx_coef_1, ldtx_coef_2)
m_mort_coef_all    <- rbind(mort_coef_1, mort_coef_2)
m_remove_coef_all  <- rbind(remove_coef_1, remove_coef_2)
mlogit_coef_all    <- rbind(mlogit_coef_1, mlogit_coef_2)
gs_mort_coef_all   <- rbind(gs_mort_coef_1, gs_mort_coef_2)
gl_coef_all        <- rbind(gl_coef_1, gl_coef_2)
dial_mort_coef_all <- rbind(dial_mort_coef_1, dial_mort_coef_2)
gl_mort_coef_all   <- rbind(gl_mort_coef_1, gl_mort_coef_2)

fwrite(m_ddtx_coef_all, paste0(serv_path, "ddtx_coef.csv"))
fwrite(m_ldtx_coef_all, paste0(serv_path, "ldtx_coef.csv"))
fwrite(m_mort_coef_all, paste0(serv_path, "mort_coef.csv"))
fwrite(m_remove_coef_all, paste0(serv_path, "remove_coef.csv"))

fwrite(mlogit_coef_all, paste0(serv_path, "mlogit_coef.csv"))
fwrite(gs_mort_coef_all, paste0(serv_path, "gs_mort_coef.csv"))
fwrite(gl_coef_all, paste0(serv_path, "gl_coef.csv"))
fwrite(dial_mort_coef_all, paste0(serv_path, "dial_mort_coef.csv"))
fwrite(gl_mort_coef_all, paste0(serv_path, "gl_mort_coef.csv"))
