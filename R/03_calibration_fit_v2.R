...
# samp_path  <- "//tsclient/Documents/Simulation Model/Model Inputs/"
out_path <- "~/Documents/R/OlderKidneyTransplantSim/results/"
# serv_path <- "C:/Users/mbkauf/Documents/Calibration/Recalibration/"

## Load model coefficients to test
# m_ddtx_coef       <- as.matrix(fread(paste0(in_path, "ddtx_coef_trim.csv")))[,-1]
# m_ddtx_shape      <- as.matrix(fread(paste0(in_path, "ddtx_shape_trim.csv")))[,-1]
# m_ldtx_coef       <- as.matrix(fread(paste0(in_path, "ldtx_coef_trim.csv")))[,-1]
# m_ldtx_shape      <- as.matrix(fread(paste0(in_path, "ldtx_shape_trim.csv")))[,-1]
# m_mort_coef       <- as.matrix(fread(paste0(in_path, "mort_coef_trim.csv")))[,-1]
# m_mort_shape      <- as.matrix(fread(paste0(in_path, "mort_shape_trim.csv")))[,-1]
# m_remove_coef     <- as.matrix(fread(paste0(in_path, "remove_coef_trim.csv")))[,-1]
# m_remove_shape    <- as.matrix(fread(paste0(in_path, "remove_shape_trim.csv")))[,-1]
# m_mlogit_coef     <- as.matrix(fread(paste0(in_path, "mlogit_coef_trim.csv")))
# m_gs_mort_coef    <- as.matrix(fread(paste0(in_path, "gs_mort_coef_trim.csv")))
# m_gs_mort_shape   <- as.matrix(fread(paste0(in_path, "gs_mort_shape_trim.csv")))[,-1]
# m_gl_coef         <- as.matrix(fread(paste0(in_path, "gl_coef_trim.csv")))
# m_gl_shape        <- as.matrix(fread(paste0(in_path, "gl_shape_trim.csv")))[,-1]
# m_dial_mort_coef  <- as.matrix(fread(paste0(in_path, "dial_mort_coef_trim.csv")))
# m_dial_mort_shape <- as.matrix(fread(paste0(in_path, "dial_mort_shape_trim.csv")))[,-1]
# m_gl_mort_coef    <- as.matrix(fread(paste0(in_path, "gl_mort_coef_trim.csv")))


## Load data set
##### Data set #####
sample_df <- as.matrix(read.csv(file = paste0(in_path, "sample_df_2.csv"),
                                sep=","))[,-1]

## Load model functions
source("~/Documents/R/OlderKidneyTransplantSim/R/02_model_functions.R", echo = TRUE)
...
pb <- txtProgressBar(min=1, max=64, style=3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress=progress)

out <- foreach(i = 1:64, .combine='rbind', .multicombine=TRUE, .options.snow=opts,
...
## Load observed data
n <- 64
obs_df  <- fread(file = paste0(in_path, "simulation_list3.csv"))
...
remove_legend$plot <- remove_legend$plot +
  ...
  theme(legend.position = 'bottom') +
  theme(strip.text.x = element_text(size = 8)) +
  scale_color_manual(breaks = c(64, 65),
                     values = c(rep("red", 64), "gray"),
...
plots_list_250 <- grid.arrange(arrangeGrob(ddtx_all$plot, ldtx_all$plot,
                                           mort_all$plot, remove_all$plot, # + theme(legend.position="none"),
                                      nrow = 2),
                          nrow=2,heights=c(10, 1))
ggsave(filename = paste0(out_path, "recalibration_list_", n.top, ".pdf"),
       plot = plots_list_250 , width = 7, height = 4)
ggsave(filename = paste0(out_path, "recalibration_list_", n.top, ".png"),
       plot = plots_list_250 , width = 7, height = 4)
...
plots_post_64 <- grid.arrange(arrangeGrob(gl_all$plot, gs_death_all$plot,
                                           gl_death_all$plot +
...
ggsave(filename = paste0(out_path, "recalibration_post_", n.top, ".pdf"),
       plot = plots_post_64, width = 7, height = 4)
ggsave(filename = paste0(out_path, "recalibration_post_", n.top, ".png"),
       plot = plots_post_64, width = 7, height = 4)
...
df_mlogit_64_all_long <- summary_df_mlogit_long(data = all_df_ddtx_64, n = n) %>%
  dplyr::mutate(outcome = ifelse(outcome == "DGF", "Delayed Graft Function",
                                 ifelse(outcome == "GL", "Graft Loss",
                                        ifelse(outcome == "Mort", "Mortality",
                                               "Functioning Graft"))))
df_glmort_64_all_long <- summary_df_glmort_long(data = all_df_ddtx_64, n = n) %>%
  dplyr::mutate(outcome = "Death at Graft Loss")
...
mlogit_64_plot <- mlogit_plot(data = df_mlogit_64_all_long, n = n)
glmort_64_plot <- glmort_plot(data = df_glmort_64_all_long, n = n)
gl_64 <- list(mlogit_64_plot, glmort_64_plot)
...
plot_64 <- gridExtra::grid.arrange(grobs=gl_64,
                                    layout_matrix = lay)
ggsave(filename = paste0(out_path, "recalibration_prop_64.pdf"),
       plot = plot_64 , width = 10, height = 4)
ggsave(filename = paste0(out_path, "recalibration_prop_64.png"),
       plot = plot_64 , width = 10, height = 4)
...
colnames(table1_all) <- c("Age at Transplant", "Sex",
                          ...
                          "OPTN 11")
write.csv(table1_all, paste0(out_path, "table1.csv"))
